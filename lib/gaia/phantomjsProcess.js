...
exports.init = function() {
	...
	exports.spawn = function(pageUrl, options) {
		...
		var privates = {
			...
			version: function(version) {
				...
				if (!semver.satisfies(current, required)) {
					exports.halt();
                    console.log('In order for this task to work properly, PhantomJS version ' + required + ' must be installed, but version ' + current + ' was detected.');
				}
			}
		};
		...
		(function loopy() {
			...
			var lines = util.read(tempfile.path).split('\n').slice(0, - 1);
			debugger
			// Re-enable logging.
		...
		function(err, result, code) {
			...
			if (code === 127) {
				console.log('In order for this task to work properly, PhantomJS must be installed locally via NPM. ' + 'If you\'re seeing this message, generally that means the NPM install has failed. ' + 'Please submit an issue providing as much detail as possible at: ' + 'https://github.com/gruntjs/grunt-lib-phantomjs/issues');
