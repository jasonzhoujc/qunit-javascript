...
exports.init = function() {
	...
	exports.spawn = function(pageUrl, options) {
		// Create temporary file to be used for gaia-phantom communication.
		...
		var privates = {
			...
			version: function(version) {
				...
				if (!semver.satisfies(current, required)) {
					exports.halt();
					gaia.log.writeln();
					gaia.log.errorlns('In order for this task to work properly, PhantomJS version ' + required + ' must be installed, but version ' + current + ' was detected.');
					gaia.warn('The correct version of PhantomJS needs to be installed.', 127);
				}
			}
		};
		...
		// 读取文件，直到halted为true，通过此判断程序是否执行完成
		(function loopy() {
			// Disable logging temporarily.
			//gaia.log.muted = true;
			// Read the file, splitting lines on \n, and removing a trailing line.
			var lines = gaia.file.read(tempfile.path).split('\n').slice(0, - 1);
			debugger
			// Re-enable logging.
			...
			var done = lines.slice(n).some(function(line) {
				...
				// Debugging messages.
				gaia.log.debug(JSON.stringify(['phantomjs'].concat(args)).magenta);
		...
		debugger
		// Keep -- PhantomJS args first, followed by gaia-specific args.
		args.push(
		// The main PhantomJS script file.
		opts.phantomScript || asset('gaia/webpageMonitor.js'),
		...
		gaia.log.debug(JSON.stringify(args));
		...
		function(err, result, code) {
			...
			cleanup();
			//gaia.verbose.or.writeln();
			//gaia.log.write('Running PhantomJS...').error();
			if (code === 127) {
				gaia.log.errorlns('In order for this task to work properly, PhantomJS must be installed locally via NPM. ' + 'If you\'re seeing this message, generally that means the NPM install has failed. ' + 'Please submit an issue providing as much detail as possible at: ' + 'https://github.com/gaiajs/gaia-lib-phantomjs/issues');
				gaia.warn('PhantomJS not found.', failCode);
			} else {
				String(result).split('\n').forEach(gaia.log.error, gaia.log);
				gaia.warn('PhantomJS exited unexpectedly with exit code ' + code + '.', failCode);
			}
			options.done(code);
		});
	};
