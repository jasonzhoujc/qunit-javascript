...
function write(msg) {
    ...
    if (!log.muted && !suppressOutput) {
        ...
        if (gaia.option('no-color')) { msg = colors.stripColors(msg); }
...
log.warn = function() {
    ...
    if (arguments.length > 0) {
        writeln('>> '.red + gaia.util._.trim(msg).replace(/\n/g, '\n>> '.red));
    } else {
        writeln('ERROR'.red);
    }
    return log;
...
log.ok = function() {
    var msg = format(arguments);
    if (arguments.length > 0) {
        writeln('>> '.green + gaia.util._.trim(msg).replace(/\n/g, '\n>> '.green));
    } else {
        writeln('OK'.green);
    }
    return log;
...
Object.keys(log).filter(function(key) {
    ...
    log.verbose[key] = function() {
        suppressOutput = !gaia.option('verbose');
...
log.wordlist = function(arr, options) {
    options = gaia.util._.defaults(options || {}, {
        separator: ', ',
        color: 'cyan'
    });
...
log.table = function(widths, texts) {
    ...
    rows.forEach(function(row) {
        ...
        for (var i = 0; i < row.length; i++) {
            ...
            if (diff > 0) { txt += gaia.util.repeat(diff, ' '); }
        }
        lines.push(txt);
    });
